...
DEFAULT_TEST_SECONDS = 60
MIN_TEST_SECONDS = 10
MAX_TEST_SECONDS = 600
WORD_COUNT = 400
...
def get_level(words_per_minute: int) -> str:
...
def evaluate_attempts(
    ...
    elapsed_seconds: int = DEFAULT_TEST_SECONDS,
    ...
    bounded_elapsed_seconds = max(1, min(elapsed_seconds, test_seconds))
    elapsed_minutes = bounded_elapsed_seconds / 60
    ...
    minute_coefficient = 60 / bounded_elapsed_seconds
    average_chars_per_correct_word = (characters / correct_words) if correct_words else 0.0
    average_chars_per_minute_per_word = average_chars_per_correct_word * minute_coefficient
    chars_per_minute = round(average_chars_per_minute_per_word * correct_words)
    words_per_minute = round(chars_per_minute / 5) if chars_per_minute else 0
    errors_per_minute = round((visited_words - correct_words) / elapsed_minutes)
    ...
    return {
        ...
        "level": get_level(words_per_minute),
    }
...
class TypingTestHandler(SimpleHTTPRequestHandler):
    ...
    def create_session(self) -> None:
        cleanup_sessions()
        requested_seconds = self.get_requested_seconds()
    ...
    def submit_result(self) -> None:
        ...
        elapsed_seconds = payload.get("elapsed_seconds", DEFAULT_TEST_SECONDS)
        test_seconds = payload.get("test_seconds", DEFAULT_TEST_SECONDS)

        if not isinstance(attempts, list):
            self.send_json({"error": "Attempts must be a list."}, HTTPStatus.BAD_REQUEST)
            return
        if not isinstance(elapsed_seconds, int):
            elapsed_seconds = DEFAULT_TEST_SECONDS
        if not isinstance(test_seconds, int):
            test_seconds = DEFAULT_TEST_SECONDS
        test_seconds = max(MIN_TEST_SECONDS, min(test_seconds, MAX_TEST_SECONDS))
        ...
        result = evaluate_attempts(session.words, attempts, elapsed_seconds, test_seconds)
        self.send_json(result)

    def get_requested_seconds(self) -> int:
        ...
        raw_seconds = query_parts.get("seconds", "")
        try:
            seconds = int(raw_seconds)
        except (TypeError, ValueError):
            return DEFAULT_TEST_SECONDS
        return max(MIN_TEST_SECONDS, min(seconds, MAX_TEST_SECONDS))
